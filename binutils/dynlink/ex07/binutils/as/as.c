...
static void writeFinalHeader(void) {
  ...
  conv4FromEcoToNative((unsigned char *) &outFileHeader.nsegs);
  conv4FromEcoToNative((unsigned char *) &outFileHeader.osyms);
  conv4FromEcoToNative((unsigned char *) &outFileHeader.nsyms);
  conv4FromEcoToNative((unsigned char *) &outFileHeader.orels);
  conv4FromEcoToNative((unsigned char *) &outFileHeader.nrels);
  conv4FromEcoToNative((unsigned char *) &outFileHeader.odata);
  conv4FromEcoToNative((unsigned char *) &outFileHeader.sdata);
...
static void writeSegmentTable(void) {
  ...
  /* record file offset */
  outFileHeader.osegs = fileOffset;
  ...
  /* update file offset */
  outFileHeader.nsegs = 3;
  fileOffset += outFileHeader.nsegs * sizeof(SegmentRecord);
}
...
static void writeSymbolTable(void) {
  /* record file offset */
  outFileHeader.osyms = fileOffset;
  /* write symbol table */
  nsyms = 0;
  walkTree(globalTable, writeSymbol);
  /* update file offset */
  outFileHeader.nsyms = nsyms;
  fileOffset += outFileHeader.nsyms * sizeof(SymbolRecord);
}
...
static void writeRelocTable(void) {
  ...
  /* record file offset */
  outFileHeader.orels = fileOffset;
  /* write reloc table */
  nrels = 0;
  f = fixupList;
  while (f != NULL) {
  ...
  /* update file offset */
  outFileHeader.nrels = nrels;
  fileOffset += outFileHeader.nrels * sizeof(RelocRecord);
}
...
static void writeData(void) {
  /* record file offset */
  outFileHeader.odata = fileOffset;
  /* write segment data */
  writeBytes(codeFile);
  writeBytes(dataFile);
  /* update file offset */
  outFileHeader.sdata = dataSize;
  fileOffset += outFileHeader.sdata;
}
...
static void writeStrings(void) {
  /* record file offset */
  outFileHeader.ostrs = fileOffset;
  /* write segment names */
  ...
  /* update file offsets */
  outFileHeader.sstrs = stringSize;
  fileOffset += outFileHeader.sstrs;
}
