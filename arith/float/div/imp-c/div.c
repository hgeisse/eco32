/*
 * div.c -- implementation and test of division
 */
...
void div24(_FP_Word x, _FP_Word y, _FP_Word *zqp, _FP_Word *zrp) {
  _FP_Word quo;
  _FP_Word rem;
  _FP_Word aux1;
  _FP_Word aux2;
...
/*
 * 24-bit divider, internal reference
 */


void div24_ref(_FP_Word x, _FP_Word y, _FP_Word *zqp, _FP_Word *zrp) {
  unsigned long lx, ly;
  _FP_Word quo, rem;
...
/*
 * 24-bit divider, check
 */


#define SKIP_MASK_24	0x00007FF0


void check_div24(void) {
  _FP_Word x, y;
  _FP_Word zq, zr;
  _FP_Word rq, rr;
  long count, errors;
  ...
  for (x = (1 << 23); x < (1 << 24); x++) {
    ...
    for (y = (1 << 23); y <= x; y++) {
      ...
      div24_ref(x, y, &rq, &rr);
      count++;
      if ((unsigned long) rq * (unsigned long) y +
          (unsigned long) rr != (unsigned long) x << 24) {
        printf("x = 0x%08X, y = 0x%08X : rq = 0x%08X, rr = 0x%08X\n",
               x, y, rq, rr);
        printf("fatal error: rq * y + rr != x << 24\n");
        exit(1);
      }
      if (rq < (1 << 24)) {
        printf("x = 0x%08X, y = 0x%08X : rq = 0x%08X, rr = 0x%08X\n",
               x, y, rq, rr);
        printf("fatal error: rq too small\n");
        exit(1);
      }
      if (rq >= (1 << 25)) {
        printf("x = 0x%08X, y = 0x%08X : rq = 0x%08X, rr = 0x%08X\n",
               x, y, rq, rr);
        printf("fatal error: rq too big\n");
        exit(1);
      }
    }
  }
...
#if DEVELOPING
_FP_Word fake_fpDiv(_FP_Word x, _FP_Word y) {
  extern _FP_Word Flags;
  extern _FP_Word Flags_ref;
  extern _FP_Word fpDiv_ref(_FP_Word x, _FP_Word y);
  _FP_Word Flags_save;
  _FP_Word z;
...
_FP_Word fpDiv(_FP_Word x, _FP_Word y) {
  _FP_Word sx, ex, fx;
  _FP_Word sy, ey, fy;
  _FP_Word sz, ez, fz;
  Bool nx;
  ...
  if (IS_NUM(ex, fx) && IS_NAN(ey, fy)) {
    ...
    /* stage 2: divide significands, compute exponent */
    shiftx = mxnorm < mynorm;
    if (debug) {
      printf("shiftx = %d\n", shiftx);
    }
    div24(mxnorm << shiftx, mynorm, &fq, &fr);
    ez = exnorm - eynorm + 127 - shiftx;
    if (debug) {
      printf("fq = 0x%08X, fr = 0x%08X, ez = %d\n", fq, fr, ez);
    }
    fz = fq >> 1;
    fzx = fq << 31;
    ...
    if ((int) ez <= 0) {
      ...
      /* Note: overflow is not possible here! */
    }
  }
  if (debug) {
    show("z", z);
...
/*
 * floating-point divider, internal reference
 */
...
int main(int argc, char *argv[]) {
  if (argc == 2 && strcmp(argv[1], "-div24") == 0) {
    printf("Check 24-bit divider\n");
    check_div24();
    return 0;
  }
