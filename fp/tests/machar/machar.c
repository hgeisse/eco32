...
void rmachar(int *ibeta, int *it, int *irnd,
             int *ngrd, int *machep, int *negep,
             int *iexp, int *minexp, int *maxexp,
             REAL *eps, REAL *epsneg,
             REAL *xmin, REAL *xmax);
...
#define DISPLAY(s,x) { \
	...
	for(i=0;i<REALSIZE;i++) printf(" %9lX ",uval.jj[i]) ; \
	printf("\n"); \
	}
...
void rmachar(ibeta,it,irnd,ngrd,machep,negep,iexp,minexp,
